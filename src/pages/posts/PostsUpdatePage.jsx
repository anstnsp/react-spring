...
const PostsUpdate = ({history, location, match}) => {
      ...
      useEffect(() => {
        ...
        async function fetchData() {
          try {
            const res = await axios.get("http://localhost:8080/api/v1/posts/"+updateKey);
            const data = res.data; 
        ...
        fetchData(); 
        return () => { isData = true } //useEffect 함수의 return값이 있는 경우 hook의 cleanup함수로 인식하고 다음 effect가 실행되기전에 실행함.
    ...
    const handleDelete = async () => {
      console.log(`## 삭제버튼 누름 ##`);
      try {
        const serverRes = await axios.delete("http://localhost:8080/api/v1/posts/"+updateKey);
    ...
    const handleSubmit = async () => {
      ...
      try {
        const result = await axios.put("http://localhost:8080/api/v1/posts/"+updateKey, updateParam);
    ...
    return (
      ...
      <AppTemplate 
          appbar={<AppBar />}
          body={<Form handleChange={handleChange} author={author} title={title} content={content}/>}  />
          <Button variant="outlined" color="primary" align="right" onClick={handleSubmit}>
            수정
          </Button>
